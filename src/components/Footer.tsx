...
import { Instagram, Send, Check, AlertCircle, ChevronDown } from 'lucide-react';
...
export default function Footer() {
  ...
  const [form, setForm] = useState({ nome: '', email: '', telefono: '', messaggio: '', corso: '' });
  const [status, setStatus] = useState<'idle' | 'loading' | 'success' | 'error'>('idle');
  const [courses, setCourses] = useState<Course[]>([]);

  useEffect(() => {
    if (!preLancio) return;
    supabase.from('courses').select('id, nome').eq('is_attivo', true).order('nome')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (preLancio && !form.corso) return;
    setStatus('loading');
    ...
    const payload = {
      ...
      messaggio:  preLancio ? null : (form.messaggio.trim() || null),
      corso:      preLancio ? form.corso : null,
    ...
    try {
      await Promise.all([
        ...
        supabase.from('contact_leads').insert({
          ...
          messaggio: preLancio ? (form.corso ? `Corso: ${form.corso}` : null) : payload.messaggio,
        }),
      ]);
      setStatus('success');
      setForm({ nome: '', email: '', telefono: '', messaggio: '', corso: '' });
    } catch {
      setStatus('error');
    }
  };
      ...
      <div className="container mx-auto">
        <div className="max-w-4xl px-6 md:px-12 pt-20 pb-16">
          <div className="space-y-12">
            ...
            {/* Form dinamico */}
            <div>
              <h4 className="font-label uppercase text-xs tracking-widest font-bold text-on-surface mb-6">
                {preLancio ? 'Iscriviti al corso' : 'Scrivici'}
              </h4>

              {status === 'success' ? (
                <div className="flex items-center gap-3 rounded-2xl px-5 py-4 border border-primary/20" style={{ background: 'rgba(181,106,86,0.07)' }}>
                  <Check size={18} className="text-primary flex-shrink-0" />
                  <div>
                    <p className="text-sm font-semibold text-on-surface">
                      {preLancio ? 'Iscrizione ricevuta!' : 'Messaggio inviato!'}
                    </p>
                    <p className="text-xs text-on-surface-variant mt-0.5">Ti contatteremo il prima possibile.</p>
                  </div>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-4 max-w-lg">
                  ...
                  {preLancio ? (
                    <div>
                      <label className={labelClass}>Corso di interesse *</label>
                      <div className="relative">
                        <select required
                          className={`${inputClass} appearance-none pr-10`}
                          value={form.corso}
                          onChange={e => setForm(f => ({ ...f, corso: e.target.value }))}
                        >
                  ...
                  >
                    {status === 'loading'
                      ...
                      : <><Send size={13} /> {preLancio ? 'Invia iscrizione' : 'Invia messaggio'}</>
                    }
                  </button>
                </form>
              )}
